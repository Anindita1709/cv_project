...
from models.build_model import build_anyloc, build_clip_selector, build_resnet, build_segmentation, default_preprocess
...
@torch.no_grad()
def select_reference_image(rgb_path: Path, ref_rgb_path: Path, clip_selector) -> Path:
    image_files = sorted([p for p in rgb_path.iterdir() if p.suffix.lower() in {".png", ".jpg", ".jpeg", ".bmp", ".webp"}])
    if not image_files:
    ...
    idx = int(np.argmin(np.linalg.norm(features - center[None, :], axis=1)))
    selected = image_files[idx]
    target = ref_rgb_path / "0.png"
...
@torch.no_grad()
def save_object_and_patch_embedding(image_path: Path, embed_path: Path, masks: list[np.ndarray], patches: np.ndarray, model) -> None:
    image = Image.open(image_path).convert("RGB")
    ...
    torch.save(object_feats, embed_path / "objects.pt")
    torch.save(patch_feats, embed_path / "patches.pt")
...
def main(config: dict) -> None:
    ...
    for scene in sorted(dataset_path.iterdir()):
        ...
        for room in sorted(scene.iterdir()):
            ...
            selected_ref = select_reference_image(rgb_path, ref_rgb_path, clip_selector)
            masks, patches = segmentation(seg_model, selected_ref, score_threshold=float(config.get("segmentation_threshold", 0.6)))
            save_object_and_patch_embedding(selected_ref, embed_path, masks, patches, object_model)
